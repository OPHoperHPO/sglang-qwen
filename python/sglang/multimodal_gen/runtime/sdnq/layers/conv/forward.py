...
def quantized_conv_forward(self, input) -> torch.FloatTensor:
    ...
    return self._conv_forward(input, dequantized_weight, bias)
...
def quantized_conv_transpose_1d_forward(self, input: torch.FloatTensor, output_size: Optional[list[int]] = None) -> torch.FloatTensor:
    ...
    if bias is not None and bias.dtype != input.dtype:
        bias = bias.to(dtype=input.dtype)
    return torch.nn.functional.conv_transpose1d(input, dequantized_weight, bias, self.stride, self.padding, output_padding, self.groups, self.dilation)
...
def quantized_conv_transpose_2d_forward(self, input: torch.FloatTensor, output_size: Optional[list[int]] = None) -> torch.FloatTensor:
    ...
    if bias is not None and bias.dtype != input.dtype:
        bias = bias.to(dtype=input.dtype)
    return torch.nn.functional.conv_transpose2d(input, dequantized_weight, bias, self.stride, self.padding, output_padding, self.groups, self.dilation)
...
def quantized_conv_transpose_3d_forward(self, input: torch.FloatTensor, output_size: Optional[list[int]] = None) -> torch.FloatTensor:
    ...
    if bias is not None and bias.dtype != input.dtype:
        bias = bias.to(dtype=input.dtype)
    return torch.nn.functional.conv_transpose3d(input, dequantized_weight, bias, self.stride, self.padding, output_padding, self.groups, self.dilation)
